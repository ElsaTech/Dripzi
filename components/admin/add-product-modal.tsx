...
// Note: Product creation is handled by Shopify Admin
...
export function AddProductModal({ isOpen, onClose }: AddProductModalProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    // Image uploads are handled in Shopify Admin
    e.preventDefault()
    alert("Product images must be managed through Shopify Admin panel.")
  }

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    alert("Product creation must be done through Shopify Admin panel.")
    onClose()
  }
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-1/2 top-1/2 z-50 max-h-[90vh] w-full max-w-2xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto p-4"
          >
            <div className="neumorphic rounded-3xl bg-white p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  <label className="mb-2 block text-sm font-semibold text-black">Images</label>
                    ...
                    {formData.images.length > 0 && (
                      <div className="mt-4 grid grid-cols-4 gap-2">
                        {formData.images.map((url, index) => (
                          <div key={index} className="aspect-square overflow-hidden rounded-xl bg-gray-100">
