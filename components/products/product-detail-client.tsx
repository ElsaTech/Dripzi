      ...
      <div className="grid gap-12 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          <motion.div
            ...
            className="neumorphic aspect-[3/4] overflow-hidden rounded-3xl"
          >
            ...
            <div className="grid grid-cols-4 gap-4">
              {product.images.map((image, index) => (
                <button
                  ...
                  className={`neumorphic-sm aspect-square overflow-hidden rounded-2xl transition-all ${
        ...
        <div className="flex flex-col">
          <div className="mb-6">
            <h1 className="mb-4 font-serif text-4xl font-bold text-black md:text-5xl">{product.name}</h1>
            <div className="flex items-center gap-4">
              {hasDiscount && (
                <span className="text-2xl text-gray-400 line-through">${regularPrice.toFixed(2)}</span>
              )}
              <span className="text-4xl font-bold text-black">${displayPrice.toFixed(2)}</span>
              {hasDiscount && (
          ...
          {hasColorOptions && (
            <div className="mb-6">
              <h3 className="mb-3 text-sm font-bold uppercase tracking-wider text-black">Color</h3>
          ...
          {hasSizeOptions && (
            <div className="mb-8">
              <h3 className="mb-3 text-sm font-bold uppercase tracking-wider text-black">Size</h3>
                ...
                {product.sizes.map((size) => {
                  return (
                    <button
                      ...
                      className={`neumorphic-sm rounded-xl py-3 font-bold transition-all ${
                        selectedSize === size
                          ? "bg-black text-white"
          ...
          <div className="mt-auto space-y-4">
            <Button
              ...
              disabled={isAdding}
              className="neumorphic w-full rounded-2xl bg-black py-6 text-lg font-bold text-white transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isAdding ? (
                "Adding..."
              ) : showSuccess ? (
            ...
            <div className="flex gap-3">
              <Button
                variant="outline"
                className="neumorphic-sm flex-1 rounded-2xl border-2 border-gray-200 bg-white py-6 font-semibold"
              >
          ...
          <div className="mt-12 space-y-6 border-t border-gray-200 pt-8">
            <div>
              <h4 className="mb-2 font-bold text-black">Product Details</h4>
            ...
            <div>
              <h4 className="mb-2 font-bold text-black">Shipping & Returns</h4>
              <p className="text-gray-600">Free shipping on orders over $100. 30-day easy returns.</p>
            </div>
          </div>
        </div>
      </div>
